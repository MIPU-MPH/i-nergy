...
def preprocessing(df):
    """
    Qualora il dataset in input non avesse la colonna delle ore ma solo una colonna di data, 
    Questa funzione provvede ad estrarre l'ora e impostare come indice la colonna delle date.
...
def split_set(df, perc_train):
    """
    Provvede a splittare in modo randomico un dataset in input.
    In output dunque avremo 2 set: quello di train e quello di validation
...
def only_train(df,perc_train = 0.6):
    """
    Qui alleniamo i modelli sul Train-set, e
    ricaviamo i 2 limiti di controllo UCL ed LCL per ogni temperatura sotto esame.
...
def only_predict(df2, codice):
    """
    Utilizza il modello ottenuto in only_train per predire sul nuovo set di dati.
    Effettua 2 tipologie di controllo: 
    1) il Residuo è contenuto nei limiti di controllo?
    2) La Rolling mean di 6 valori di Residui contigui sono > 6*Sigma residui validation?
...
""" 
in input vogliamo un dataframe con colonne le variabili di interesse
e, in particolare, come ultima colonna, l'ora corrispondente alla specifica riga.

L'indice non è importante, ma se volessimo utilizzare l'informazione del TimeStamp
allora dobbiamo necessariamente inserirla come indice e mai come colonna.

Come agire:

                        ''Funzione A (Train):''

codice_personale = only_train(df)

                        ''Funzione B (Predict,e alert):''

only_predict(df2, codice_personale)
