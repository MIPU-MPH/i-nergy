...
def sendPredRequest(api_url, CSV_path, path):
    #-------------- inzio
    ...
    #---------------------- inizio
    """ funzione he dal dataframe caricato da csv, da come output il corrispettivo file json"""
    from numpy import random
    jsonreq = {}
    jsonreq['data'] = []

  
    for i in range(0,len(df)): # creo le chiavi principali, tante quanti sono gli intervalli di 5 minuti in 24 ore
            diz_value = {}
            for col in df.columns[:-1]:
                diz_value[col] = df[col][i]
            diz_value['hour'] = int(df.iloc[i][-1])
            diz_value['time'] = df.index[i]
            jsonreq['data'].append(diz_value)
    #---------------------- fine
...
if __name__ == '__main__':
    ...
    try:
        ...
        jsonreq = json.load(f)
        Richiesta = sendPredRequest(api_url, jsonreq, output_folder)
        df_0 = Richiesta[0]
        df = Richiesta[1]
